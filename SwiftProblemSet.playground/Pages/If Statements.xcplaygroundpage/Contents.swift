...
func checkForRelease(bugs: Int, music: Bool, levels: Int) {
    ...
    if bugs >= 10 {
        ...
        print("\(game) is not ready for sale, the number of bugs are \(bugs)")
    }
    
    if !hasMusic{
        
        print("\(game) is not ready for sale, music is required for the game")
        
    }
    
    if levels <= 5{
        print("\(game) is not ready for sale, the levels of the game are \(levels)")
...
func checkTrainingStatus(name: String, bike: Bool, run: Bool) {
    ...
    if bike && run {
        ...
        print("\(name) is not ready, as run cannot be finished")
    }else if !bike && run {
        print("\(name) is not ready, as bike cannot be finished")
    }else if !bike && !run {
        print("\(name) is not ready, as bike and run cannot be finished")
    }
